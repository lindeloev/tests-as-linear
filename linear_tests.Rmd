...
Most of the common statistical models (t-test, correlation, ANOVA; chi-square, etc.) are special cases of linear models or a very close approximation. This beautiful simplicity means that there is less to learn. In particular, it all comes down to $y = a \cdot x + b$ which most students know from highschool. Unfortunately, stats are usually taught as if each test is an independent tool, needlessly making life more complicated for students and teachers alike. 

This needless complexity multiplies when students try to rote learn the parametric assumptions underlying each test separately rather than deducing them from the linear model. 

For this reason, I think that teaching linear models first and foremost and *then* name-dropping the special cases along the way makes for an excellent teaching strategy, emphasizing *understanding* over rote learning. Concerning the teaching of non-parametric tests in intro-courses, I think that we can justify [lying-to-children](https://en.wikipedia.org/wiki/Lie-to-children) and teach non-parametric tests as if they are merely ranked versions of the parametric tests, which is a close approximation but not exact. After all, even mainstream stats are approximations from the almost-universally-used [Bessel's correction](https://en.wikipedia.org/wiki/Bessel%27s_correction) to the almost-never-perfectly satisfied parametric assumptions.

Use the menu to the left to jump to your favourite section. I present this in three parts:

1. A table to give an overview and to raise awareness.
2. Actual R code and more details about these equivalences.
3. A [course outline](#course) as inspiration for stats teachers.

I hope that others will join in and refine this list as well as add further tests to it. I'm not a statistician and there are likely some inaccuracies or errors here and there.


# Overview of tests
This table to show the underlying linear models of common tests. Formulating all the tests in the same language also highlights similarities between them.
<!--
...
Get it [as an image](linear_tests_cheat_sheet.png) or [as a PDF](linear_tests_cheat_sheet.pdf).
...
The correlation coefficient of the linear model is identical to a "real" Pearson correlation, but p-values are an approximation which is is [appropriate for samples greater than N=10 and almost perfect when N > 20](https://rpubs.com/lindeloev/spearman_pearson). 
...
df = bind_rows(at, bt, ct) %>%
  mutate(model = c('cor.test', 'lm', 'lm scaled')) %>%
  rename(t = statistic, 
         r = estimate) %>%
  select(model, r, p.value, t, conf.low, conf.high)
...
```{r, echo=FALSE}
bt = tidy(b)[2, ]  # Only slope
df = data.frame(model = c('cor.test', 'lm'), 
               p.value = c(a$p.value, bt$p.value),
               rho = c(a$estimate, bt$estimate))
...
[This approximation is appropriate when the sample size is larger than 14 and almost perfect if the sample size is larger than 50](https://rpubs.com/lindeloev/wilcoxon-ttest).
...
df = bind_rows(at, bt) %>%
  mutate(model = c('t.test', 'lm')) %>%
  rename(mean = estimate,
         t = statistic) %>%
  select(model, mean, p.value, t, df, conf.low, conf.high)
print_df(df)
```
...
```{r, echo=FALSE}
bt = tidy(b)
df = data.frame(model=c('wilcox.test', 'lm', 't.test'),
               p.value=c(a$p.value, bt$p.value, c$p.value),
               mean_rank = c(NA, bt$estimate, c$estimate))
print_df(df)
```
...
df = bind_rows(at, bt) %>%
  ...
  mutate(model = c('t.test', 'lm')) %>%
  select(model, mean, p.value, df, t, conf.low, conf.high)
print_df(df)
```
...
```{r, echo=FALSE}
# Tidy data for individual tests
at = tidy(a)
bt = tidy(b)
bt$mean_rank_diff = bt$estimate
ct = tidy(c)
ct$mean_rank_diff = ct$estimate

# Print nicely
df = bind_rows(at, bt, ct) %>%
  mutate(model=c('wilcox.test', 'lm', 't.test')) %>%
  select(model, p.value, mean_rank_diff)
print_df(df)
```
...
To me, equivalences like this make "non-parametric" statistics much easier to understand. The approximation is appropriate [when the sample size is larger than 11 in each group and virtually perfect when N > 30 in each group](https://rpubs.com/lindeloev/mann_ttest).
...
If a data point was sampled from the first group, i.e., when $x_i = 0$, the model simply becomes $y = \beta_0$. In other words, the model predicts that that data point is $beta_0$. It turns out that the $\beta$ which best describes a set of data points is the *mean* of those data points, so $\beta_0$ is the mean of group 1.

On the other hand, data points sampled from the second group would have $x_i = 1$ so the model becomes $y_i = \beta_0 + \beta_1$. In other words, we add $\beta_1$ to "shift" from the mean of the first group to the mean of the second group. Thus $\beta_1$ becomes the *difference* between the groups. 

As an example, say group 1 is 25 years old ($\beta_0 = 25$) and group 2 is 28 years old ($\beta_1 = 3$), then the model for a person in group 1 is $y = 25 + 3 \cdot 0 = 25$ and the model for a person in group 2 is: $y = 25 + 3 \cdot 1 = 28$. Hooray, it works! For first-timers it takes a few moments to understand dummy coding, but you only need to know addition and multiplication to get there!
...
# Print it nicely
df = bind_rows(at, bt[2,]) %>%
  mutate(model=c('t.test', 'lm')) %>%
  rename(mean_y = estimate1,
         difference = estimate2) %>%
  select(model, mean_y, difference, p.value, df, conf.low, conf.high)
print_df(df)
```
...
df = data.frame(
  ...
  p.value = c(a$p.value, bt$p.value[2]),
  rank_diff = c(NA, bt$estimate[2])
)
print_df(df)
```
...
df = data.frame(
  model = c('t.test', 'gls'),
  mean_y = c(at$estimate1, bs$coefficients[1]),
  mean_diff = c(at$estimate2 - at$estimate1, bs$coefficients[2]),
  p.value = c(at$p.value, coef(bs)[2, 4]),
  t = c(at$statistic, -coef(bs)[2, 3]),
  conf.low = c(at$conf.low, -confint(b)[2,2]),
  conf.high = c(at$conf.high, -confint(b)[2,1])
)
print_df(df)
```
<div class='fold o'>
...
Again, if you do this for just one or two groups, we're already acquainted with those equations, i.e. the [Wilcoxon signed-rank test](#t1) or the [Mann-Whitney U test](#t2) respectively. 
...
```{r, echo=FALSE}
at = tidy(a)[1,]
at$df.residual = a$Df[2]
bt = glance(b)

df = bind_rows(at, bt) %>%
  mutate(model = c('Anova', 'lm')) %>%
  rename(F = statistic) %>%
  select(model, F, p.value, df, df.residual)
print_df(df)
```
...
df = bind_rows(et, ft, at) %>%
  mutate(model=c('lm', 'lm', 'Anova', 'Anova'),
         term = c('group', 'mood', 'mood', 'group')) %>%
  rename(F = statistic) %>%
  select(term, model, F, p.value) %>%
  arrange(term, model)
print_df(df)
```
...
Now let's see that the Goodness of fit is just a log-linear equivalent to a one-way ANOVA:
...
c = glm(counts ~ mood, data=D, family=poisson(link='log'))
```
...
at = tidy(a)
at$df = at$parameter
bt = tidy(b)[2, ]  # Not intercept
bt$statistic = bt$Rao

df = bind_rows(at, bt) %>%
  mutate(model = c('chisq.test', 'glm LRT')) %>%
...
1. **Fundamentals of regression:**
    ...
    6. Briefly introduce $R^2$ and that for the simple regression models above and mention in passing that this is called [the Pearson and Spearman correlation coefficients](#correlation). 
...
3. I have not discussed inference. I am only comparing p-values as a crude way to show the equivalences between the underlying models since people care about p-values. Parameter estimates will show the same equivalence. How to do *inference* is another matter. I am personally a Bayesian, but this would be less accessible to the wider audience. Also, doing [robust models](https://en.wikipedia.org/wiki/Robust_statistics) would be preferable, but fail to show the equivalence.

4. Several named tests are still missing from the list and will hopefully be added at a later time: the Sign test (require large N to be reasonably approximated by a linear model), Friedman as RM-ANOVA on `rank(y)`, McNemar, ANCOVA, and Binomial. Feel free to submit "solutions"!


Others have presented bits and pieces of this puzzle here and there. Notably:
 ...
 * [This question and replies](https://stats.stackexchange.com/questions/59047/how-are-regression-the-t-test-and-the-anova-all-versions-of-the-general-linear) at StackOverflow
 * [These slides by Christoph Scheepers here](https://www.uni-tuebingen.de/fileadmin/Uni_Tuebingen/SFB/SFB_833/A_Bereich/A1/Christoph_Scheepers_-_Statistikworkshop.pdf) on Chi-Square as log-linear.
