...
This document presents the close relationship between the p-values of the Mann-Whitney U test and a two-sample t-test on the ranked data. It is an appendix to the post "[Common statistical tests as linear models](https://lindeloev.github.io/tests-as-linear)".

**TL;DR: I argue below that starting at N = 12, 5the t-test is a reasonable approximation. For N > 30, it is almost exact.**

Since the Mann-Whitney U is a Wilcoxon test, it may also be worth looking at [my equivalent simulation](https://lindeloev.github.io/tests-as-linear/simulations/wilcoxon_ttest) about whether Wilcoxon Signed Rank test is one-sample t-test on ranks (hint: almost yes).
