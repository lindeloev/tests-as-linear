...
For this reason, I think that teaching linear models first and foremost and *then* name-dropping the special cases along the way makes for an excellent teaching strategy, emphasizing *understanding* over rote learning. Concerning the teaching of non-parametric tests in intro-courses, I think that we can justify [lying-to-children](https://en.wikipedia.org/wiki/Lie-to-children) and teach non-parametric tests as if they are merely ranked versions of the parametric tests. I have supplemented this document with links to analyses of how accurate the non-parametric-as-ranked-parametric approximation is for different sample sizes (spoiler: quite accurate for N > 15). After all, even mainstream stats are approximations, from the almost-universally-used [Bessel's correction](https://en.wikipedia.org/wiki/Bessel%27s_correction) to the never-perfectly-satisfied parametric assumptions.

Use the menu to jump to your favourite section. I hope that you will join in suggesting improvements or submitting improvements yourself in [the Github repo to this page](https://github.com/lindeloev/tests-as-linear). Let's make it awesome!
...
P_pearson = ggplot(D_correlation, aes(x=X1*0.5+0.2, y=X2*0.5+0.4)) +
  ...
  scale_color_manual(name=NULL, values=c("blue", "red"), labels=c(bquote(Î²[0]*" (intercept)"), bquote(Î²[1]*" (slope)")))
...
The correlation coefficient of the linear model is identical to a "real" Pearson correlation, but p-values are an approximation which is is [appropriate for samples greater than N=10 and almost perfect when N > 20](simulate_spearman.html).
...
P_t1 = ggplot(D_t1, aes(y=y, x=0)) + 
  ...
  scale_color_manual(name=NULL, values=c("blue"), labels=c(bquote(Î²[0]*" (intercept)"))) +
...
P_t2 = ggplot(D_t2, aes(x=x, y=y)) + 
  ...
  scale_color_manual(name=NULL, values=c("blue", "red", "darkblue"), labels=c(bquote(Î²[0]*" (group 1 mean)"), bquote(Î²[1]*" (slope = difference)"), bquote(Î²[0]+Î²[1]%.%1*" (group 2 mean)")))
...
P_anova1 = ggplot(D_anova1, aes(x=x, y=y)) + 
  ...
  scale_color_manual(
    ...
    labels=c(
      bquote(Î²[0]*" (group 1 mean)"), 
      bquote(Î²[1]*", "*Î²[2]*",  etc. (slopes/differences to "*Î²[0]*")"),
      bquote(Î²[0]*"+"*Î²[1]*", "*Î²[0]*"+"*Î²[2]*", etc. (absolute intercepts)")
    )
  )
...
This approximation is [good enough for 12 or more data points](simulate_kruskall.html). Again, if you do this for just one or two groups, we're already acquainted with those equations, i.e. the [Wilcoxon signed-rank test](#t1) or the [Mann-Whitney U test](#t2) respectively. 
